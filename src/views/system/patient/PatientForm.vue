...
defineOptions({ name: 'SystemPatientForm' })
...
const formRules = reactive<FormRules>({
  username: [
    { required: true, message: '用户账号不能为空', trigger: 'blur' },
    {
      ...
      message: '支持输入数字或英文字母，且长度在20个字符内',
      trigger: 'blur'
    }
  ],
  deptId: [{ required: true, message: '归属组织不能为空', trigger: 'change' }],
  nickname: [{ required: true, message: '用户名称不能为空', trigger: 'blur' }],
  password: [
    { required: true, message: '用户密码不能为空', trigger: 'blur' },
    {
      pattern: /^(?:[^\u4e00-\u9fa5]){8}$/,
      message: '请输入8位密码，支持数字、字母、特殊字符',
      trigger: 'blur'
    }
  ],
  // email: [
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await PatientApi.getPatient(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as PatientApi.PatientVO
