...
export const getUploadUrl = (): string => {
  return import.meta.env.VITE_API_URL + '/infra/file/upload'
}
...
export const useUpload = (directory?: string) => {
  ...
  const httpRequestAvatar = async (options: UploadRequestOptions) => {
    return new Promise((resolve, reject) => {
      FileApi.updateAvatar({avatarFile: options.file})
        .then((res) => {
          if (res.code === 0) {
            resolve(res)
          } else {
